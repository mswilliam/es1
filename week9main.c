...
void Delay(void){unsigned long volatile time;
  time = 71933; // 0.05sec in simulation
  while(time){
   time--;
  }
}
int IsSw1Pressed(){
	return ((GPIO_PORTF_DATA_R & MASK_BIT_4) == 0x00);
}
int IsSw2Pressed(){
	return ((GPIO_PORTF_DATA_R & MASK_BIT_0) == 0x00);
}
void TurnOffLed(void){
	GPIO_PORTF_DATA_R &= ~MASK_BIT_1;
}
...
int main(void){  unsigned long i,last,now;
  ...
  EnableInterrupts();           // enable interrupts for the grader
	oldData = GPIO_PORTF_DATA_R&(MASK_BIT_4|MASK_BIT_1|MASK_BIT_0);
  while(1){
    ...
    if(i<50){
      now = NVIC_ST_CURRENT_R;
				...
				Data[i] = GPIO_PORTF_DATA_R&(MASK_BIT_4|MASK_BIT_1|MASK_BIT_0); // record PF1
				i++;
			}
      last = now;
    }
