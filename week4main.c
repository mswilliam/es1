...
#define PORT_BIT_0 0x01
#define PORT_BIT_1 0x02
#define PORT_BIT_2 0x04
#define PORT_BIT_3 0x08
...
int main(void){    
  ...
  while(1){
    SW1 = GPIO_PORTF_DATA_R&PORT_BIT_4;     // read PF4 into SW1
    SW2 = GPIO_PORTF_DATA_R&PORT_BIT_0;     // read PF0 into SW2
    if(!SW1 && !SW2){                     // both pressed
      GPIO_PORTF_DATA_R |= PORT_BIT_2;       // LED is blue
      GPIO_PORTF_DATA_R &= ~(PORT_BIT_1|PORT_BIT_3);       
    } else{                           
      if((!SW1)&&SW2){                // just SW1 pressed
        GPIO_PORTF_DATA_R |= PORT_BIT_1;     // LED is red
				GPIO_PORTF_DATA_R &= ~(PORT_BIT_2|PORT_BIT_3);       
      } else{                        
        if(SW1&&(!SW2)){              // just SW2 pressed
          GPIO_PORTF_DATA_R = PORT_BIT_3;   // LED is green
					GPIO_PORTF_DATA_R &= ~(PORT_BIT_1|PORT_BIT_2); 
        }else{                        // neither switch
          GPIO_PORTF_DATA_R &= ~(PORT_BIT_1|PORT_BIT_2|PORT_BIT_3);   // LED is off
        }
      }
    }
  }
